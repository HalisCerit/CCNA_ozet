...
- 	Ağda bulunan son kullanıcı cihazları **host** ya da **end devices** kavramlarıyla  ifade edilir. Pcler, ağa bağlı yazıcılar, VOIP’ler, telefonlar vb. gibi IP üzerinden haberleşme yapan end device'lar **NIC (Network Interface Card)** ile networke bağlanırlar. NIC’in diğer bir adı da **Ethernet Card**’dır.
...
- NIC'in neredeyse her host cihazında bulunur. NIC intermediary device'lardan router ve firewall'da bulunurken switch'de bulunmaz. Ancak yerel ağda cihazlar switchler bağlanmak için ethernet kartlarını kullanırlar. Yani switchlerin NIC kartı yoktur ancak NIC'ler bağlanmak için swicthleri kullanır.
...
- WLAN (Wireless LAN).
- SAN (Storage Area Network): Sunucuların, hard disk sunucusu ile olan ağı. Ancak artık günümüzde hard diskler sunucuların içerisinde bulunmaktadır bu teknolojiye **hyperconversion** denilmektedir . 
